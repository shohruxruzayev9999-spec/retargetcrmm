...
export function computeProjectMetrics(project) {
  ...
  const overdueTasks   = tasks.filter(t => t.deadline && t.deadline < today && t.status !== "Bajarildi").length;
...
export function computeProjectMemberStats(project) {
  ...
  for (const task of (project.tasks || [])) {
    ...
    s.total++;
    if (task.status === "Bajarildi") s.completed++;
    if (task.status === "Tasdiqlandi") s.approved++;
    if (task.status === "Jarayonda" || task.status === "Ko'rib chiqilmoqda") s.active++;
    if (task.deadline && task.deadline < today && task.status !== "Bajarildi") s.overdue++;
  }
  return stats;
}
