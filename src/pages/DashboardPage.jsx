  ...
  return (
    <Card style={{ marginTop: 22, overflow: "hidden" }}>
      ...
      <div style={{ marginTop: 22, position: "relative", paddingTop: 18 }}>
        <div style={{ height: 22, borderRadius: T.radius.full, background: "linear-gradient(90deg, #dc2626 0%, #f59e0b 45%, #16a34a 100%)", overflow: "hidden", border: `1px solid ${T.colors.border}` }} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", marginTop: 8, color: T.colors.textMuted, fontSize: 11, fontWeight: 700 }}>
          {["Past", "Xavf", "Nazorat", "Yaxshi", "A'lo"].map(item => <div key={item}>{item}</div>)}
        </div>
        <div style={{ position: "absolute", top: 0, left, transform: "translateX(-50%)", width: 18, height: 50, display: "flex", alignItems: "flex-start", justifyContent: "center" }}>
