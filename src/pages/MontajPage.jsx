...
function buildVideoMetrics(tasks, employeeMap) {
  ...
  tasks.forEach((task) => {
    ...
    if (task.status === "Montajda") metric.videoActive += 1;
...
export const MontajPage = memo(function MontajPage({
  ...
  const summary = useMemo(
    () => ({
      ...
      ready: monthScopedTasks.filter((task) => task.status === "Tasdiqlandi").length,
      active: monthScopedTasks.filter((task) => task.status === "Montajda").length,
      review: monthScopedTasks.filter((task) => task.status === "Review").length,
    }),
    [monthScopedTasks]
  );
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14, marginBottom: 18 }}>
        ...
        <Card style={{ padding: 18 }}>
          ...
          <div style={{ marginTop: 4, color: T.colors.textSecondary, fontSize: 12 }}>Tasdiqlanib yakunlangan videolar</div>
        </Card>
        <Card style={{ padding: 18 }}>
        ...
        <Card style={{ padding: 18 }}>
          ...
          <div style={{ marginTop: 8, fontWeight: 800 }}>Review kutmoqda</div>
      ...
      <Card style={{ marginBottom: canEdit(profile.role) ? 18 : 0 }}>
        ...
        {filteredTasks.length ? (
          <DataTable columns={["#", "Video nomi", "Loyiha", "Format", "Deadline", "Montajor", "Status", "Revision", "Amal"]}>
            {filteredTasks.map((task, index) => {
              ...
              const canMutateOwnTask = canEditOwnVideoTask(profile, task) && canWork && ["Montajda", "Revision"].includes(task.status);
              ...
              const showApproveActions = canApproveVideo(profile.role) && task.status === "Review" && canWork;
                  ...
                  <Cell>
                    <StatusSelect
                      ...
                      onChange={(status) => {
                        ...
                        if (canMutateOwnTask && status === "Review") {
                          updateVideoTask(task, { status: "Review" });
                        }
                      }}
                    />
                  </Cell>
              ...
              {editorCards.map(({ employee, metrics, deadlineRate, firstApprovalRate, kpi }) => (
                <Card key={employee.id} style={{ padding: 18 }}>
                  ...
                  <div style={{ borderTop: `1px solid ${T.colors.borderLight}`, paddingTop: 12, display: "grid", gap: 8, fontSize: 13 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: T.colors.textSecondary }}>Revision:</span><strong>{metrics.videoRevision || 0} ta</strong></div>
                  ...
                  <div style={{ marginTop: 14, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                    ...
                    <StatusBadge value={kpi >= 70 ? "Tasdiqlandi" : kpi >= 45 ? "Jarayonda" : "Kutilmoqda"} />
                  </div>
