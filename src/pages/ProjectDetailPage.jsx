...
export const TasksTab = memo(function TasksTab({ profile, project, employees, selectedMonthId, editable, onUpdateProject }) {
  ...
  function openForEdit(task) {
    ...
    setDraft({
      ...
      status: videoTask ? normalizeMontajStatus(task.status) : (task.status || "Rejalashtirildi"),
      note: task.note || "",
  ...
  function updateDraftFormat(value) {
    setDraft((prev) => {
      ...
      const currentStatus = normalizeMontajStatus(prev.status);
      const nextStatus = nextIsVideo
        ? (MONTAJ_STATUSES.includes(currentStatus) ? currentStatus : "Kutilmoqda")
  ...
  function saveTask() {
    ...
    const normalizedVideoStatus = normalizeMontajStatus(draft.status);
    const normalizedDraft = {
      ...draft,
      format: draft.format || "",
      montajorId: isVideoTask(draft.format) ? (draft.montajorId || "") : "",
      status: isVideoTask(draft.format)
        ? (MONTAJ_STATUSES.includes(normalizedVideoStatus) ? normalizedVideoStatus : "Kutilmoqda")
  ...
  function renderTaskRows(tasks) {
    return tasks.map((task) => {
      ...
      return (
        <Row key={task.id}>
          <Cell style={{ fontWeight: 800, minWidth: 340 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 280 }}>{task.name}</div>
                {task.format ? <div style={{ marginTop: 4, fontSize: 11, fontWeight: 800, color: T.colors.accent, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{task.format}</div> : null}
                {assignedEditor ? <div style={{ marginTop: 4, fontSize: 11, color: T.colors.textSecondary, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Montajor: {assignedEditor.name}</div> : null}
                {task.note ? <div style={{ marginTop: 4, fontSize: 12, color: T.colors.textMuted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 280 }}>{task.note}</div> : null}
              </div>
            </div>
          </Cell>
          <Cell style={{ minWidth: 190 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {owner ? <Avatar name={owner.name} url={owner.avatarUrl} size={28} /> : null}
              <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{owner?.name || "Biriktirilmagan"}</span>
            </div>
          </Cell>
          <Cell style={{ minWidth: 130, whiteSpace: "nowrap" }}>{task.start || "-"}</Cell>
          <Cell style={{ minWidth: 130, fontWeight: 800, color: overdue ? T.colors.red : T.colors.text, whiteSpace: "nowrap" }}>{task.deadline || "-"}</Cell>
          <Cell style={{ minWidth: 170 }}>
            <StatusSelect
              value={displayStatus}
              options={videoTask ? MONTAJ_STATUSES : TASK_STATUSES}
              onChange={canChangeStatus ? (status) => updateTaskStatus(task.id, videoTask ? normalizeMontajStatus(status) : status) : null}
              disabled={!canChangeStatus}
            />
          </Cell>
          <Cell style={{ minWidth: 220, color: T.colors.textMuted }}>
            <div style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 200 }}>{task.note || "—"}</div>
          </Cell>
          <Cell style={{ minWidth: 190 }}>
          ...
          <Cell style={{ minWidth: 170, whiteSpace: "nowrap" }}>
            {sectionEditable ? (
